...
def stage_b(num, length, udp_port, secret_a, student_id):
    ...
    while True:
        padding = 4 - (length % 4)
        data, client_addr = listener.recvfrom(12 + (length + 4) + padding)
        # Length of payload stored in first 4 bytes (header is 12 bytes)
        payload_int = int.from_bytes(data[:4], byteorder='big')
        # if len(data) - 12 != payload_int + 4 + padding:
        ...
        headerCheck = check_header(data[:12], length + 4, secret_a)
        ...
        last_bytes = data[16:]
        # if not b'\0' * (length + padding) != data[16:]:
        #     print (b'\0' * (length + padding))
        #     print (data[16:])
